...
significant.variables <- coefficients.df %>% 
   dplyr::select(last_col()) <= 0.15  # What happens if you tweak this???
...
\tiny
```{r, warning=FALSE}
classtrain3 <- (as.numeric(classtrain)-1)
classval3 <- (as.numeric(classval)-1)

dtrain <- xgb.DMatrix(data = as.matrix(train.fs), label = classtrain3)
dval <- xgb.DMatrix(data = as.matrix(validation.fs), label = classval3)
...
rankingXGBtrain <- predict(xgbModel,data.matrix(train.fs),
                            type="response") 

# Predict validation (OUTPUTS PROBABILITIES)
rankingXGBval <- predict(xgbModel,data.matrix(validation.fs),
                          type="response") 
```
...
```{r}
test.fs <- test %>% select_if(significant.variables)

# Predict the test data (OUTPUTS LOG-ODDS) to get rankings
rankingXGBtest <- predict(xgbModel, data.matrix(test.fs))
